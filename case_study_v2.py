...
# outliers clustering
# use appterns and average representation vector on all the co-occurrence noun and the current outlier
OUTLIER_LABEL = -1
MIN_COUNT = 5
...
def combined_clustering(sim_matrix_a, sim_matrix_b, outliers):
    label_to_outliers = defaultdict(list)
    clustering_a = np.argmax(sim_matrix_a, axis=1)
    max_sim_a = np.amax(sim_matrix_a, axis=1)
    clustering_b = np.argmax(sim_matrix_b, axis=1)
    max_sim_b = np.amax(sim_matrix_b, axis=1)

    labels_a = clustering_a.tolist()
    labels_b = clustering_b.tolist()
    for idx,label_a in enumerate(labels_a):
        clustered_outlier = outliers[idx]
        if clustered_outlier == "pool":
            print "restaurant"
        if max_sim_a[idx] > max_sim_b[idx]:
            label_to_outliers[label_a].append(clustered_outlier)
        else:
            label_to_outliers[labels_b[idx]].append(clustered_outlier)
    return label_to_outliers
...
def run():
    ...
    label_to_outliers_2 = cluster(patterns_similarity_matrix, outliers_list)
    generate_output(label_to_nouns, label_to_outliers_2, "patterns")

    label_to_outliers_3 = combined_clustering(similarity_matrix, patterns_similarity_matrix, outliers_list)
    generate_output(label_to_nouns, label_to_outliers_3, "max_sim_patterns")
